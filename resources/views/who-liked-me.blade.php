...
<div class="max-w-5xl mx-auto">
    <div class="mb-5">
        ...
        <p class="text-sm text-gray-400 mt-0.5" data-en="Like them back to start a conversation" data-jp="いいね返しで会話が始まります">Like them back to start a conversation</p>
    </div>
            ...
            @foreach($likers as $liker)
                <div class="card overflow-hidden animate-in" style="animation-delay:{{ $loop->index * 0.04 }}s">
                    <a href="{{ route('user.profile', $liker->id) }}" class="block">
                        <div class="relative h-48 bg-gray-100 dark:bg-gray-800">
                            @if($liker->profile && $liker->profile->profile_photo)
                                <img src="{{ asset('storage/' . $liker->profile->profile_photo) }}" class="w-full h-full object-cover" loading="lazy">
                            @else
                    ...
                    <div class="p-3.5">
                        ...
                        @if($liker->profile)<p class="text-[11px] text-gray-400 mt-0.5">{{ $liker->profile->location }}</p>@endif
    ...
    @else
        <div class="text-center py-20">
            ...
            <p class="text-xs text-gray-400 mb-4" data-en="Make sure your profile is complete — it helps you get noticed." data-jp="プロフィールを充実させると、注目されやすくなります。">Make sure your profile is complete — it helps you get noticed.</p>
