...
<div class="max-w-4xl mx-auto space-y-8">
    ...
    <div class="animate-in">
        ...
        <p class="text-sm text-gray-500 mt-0.5" data-en="here's what's happening today." data-jp="今日の状況です。">here's what's happening today.</p>
    </div>
        ...
        <div class="bg-rose-50 dark:bg-rose-900/10 border border-rose-200/40 dark:border-rose-800/30 rounded-xl p-5 animate-in" style="animation-delay:.05s">
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-300 mt-0.5 leading-relaxed" data-en="Browse profiles and like someone to create your first match." data-jp="プロフィールを見ていいねしましょう。">Browse profiles and like someone to create your first match.</p>
        ...
        <div class="bg-amber-50 dark:bg-amber-900/10 border border-amber-200/40 dark:border-amber-800/30 rounded-xl p-5 animate-in" style="animation-delay:.05s">
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-300 mt-0.5" data-en="Like them back to create a match and start chatting." data-jp="いいね返しでマッチが成立します。">Like them back to create a match and start chatting.</p>
        ...
        <div class="bg-violet-50 dark:bg-violet-900/10 border border-violet-200/40 dark:border-violet-800/30 rounded-xl p-5 animate-in" style="animation-delay:.05s">
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-300 mt-0.5" data-en="Don't keep them waiting!" data-jp="返信しましょう！">Don't keep them waiting!</p>
    ...
    <div class="grid grid-cols-3 gap-3 animate-in" style="animation-delay:.1s">
        <a href="{{ route('search') }}" class="card p-4 text-center">
            ...
            <div class="text-[11px] text-gray-500 mt-0.5" data-en="Matches" data-jp="マッチ">Matches</div>
        </a>
        <a href="{{ route('who.liked.me') }}" class="card p-4 text-center">
            ...
            <div class="text-[11px] text-gray-500 mt-0.5" data-en="Likes" data-jp="いいね">Likes</div>
        </a>
        <a href="{{ route('messages.inbox') }}" class="card p-4 text-center">
            ...
            <div class="text-[11px] text-gray-500 mt-0.5" data-en="Unread" data-jp="未読">Unread</div>
        </a>
    </div>
                        ...
                        <a href="{{ route('user.profile', $profile->id) }}" class="card flex items-center gap-4 p-4 group">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                @if($profile->profile && $profile->profile->prefecture)
                                    <p class="text-xs text-gray-500 mt-0.5">{{ $profile->profile->prefecture }}@if($profile->profile->location) &middot; {{ $profile->profile->location }}@endif</p>
                                @endif
                            </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="animate-in" style="animation-delay:.2s">
                <div class="card p-4">
                    ...
                    <p class="text-[11px] text-gray-500 mt-2" id="daily-quote-author"></p>
                </div>
            </div>
            ...
            <div class="animate-in" style="animation-delay:.3s">
                ...
                <div class="space-y-2.5 text-xs text-gray-500 dark:text-gray-300">
...
{{-- Game Modal --}}
<div id="game-modal" style="display:none" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
    ...
    <div class="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg p-0 animate-in max-h-[90vh] overflow-hidden border border-gray-200 dark:border-gray-800">
        <button onclick="closeGameModal()" class="absolute top-3 right-3 z-10 text-gray-500 hover:text-gray-600 dark:hover:text-gray-200 bg-white/80 dark:bg-gray-800/80 backdrop-blur rounded-full w-7 h-7 flex items-center justify-center">
...
{{-- Blog Modal --}}
<div id="blog-modal" style="display:none" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
    ...
    <div class="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-2xl p-0 animate-in max-h-[90vh] overflow-hidden border border-gray-200 dark:border-gray-800">
        <button onclick="closeBlogModal()" class="absolute top-3 right-3 z-10 text-gray-500 hover:text-gray-600 dark:hover:text-gray-200 bg-white/80 dark:bg-gray-800/80 backdrop-blur rounded-full w-7 h-7 flex items-center justify-center">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
        </button>
...
// MODAL HELPERS
// ═══════════════════════════════════════
function closeGameModal(){catchState.active=false;if(catchState.interval)clearInterval(catchState.interval);if(catchState.spawnInterval)clearTimeout(catchState.spawnInterval);if(emojiState.interval)clearInterval(emojiState.interval);document.getElementById('game-modal').style.display='none'}
function openGameModal(h){const gc=document.getElementById('game-content');gc.innerHTML=h;document.getElementById('game-modal').style.display='flex'}
function closeBlogModal(){document.getElementById('blog-modal').style.display='none'}
...
setTimeout(()=>{openGameModal(`<div class="relative"><div class="confetti-container" id="catch-confetti"></div><div class="bg-gradient-to-b from-emerald-500 to-teal-500 p-6 text-center text-white"><div class="text-3xl mb-2">${emoji}</div><h3 class="text-lg font-bold">${l==='en'?"Time's Up!":"タイムアップ！"}</h3><p class="text-white/70 text-sm">${msg}</p></div><div class="p-6 text-center"><div class="inline-flex items-center gap-1 bg-emerald-50 dark:bg-emerald-900/15 rounded-full px-5 py-3 mb-2"><span class="text-2xl font-black text-emerald-500 tabular-nums">${s}</span><span class="text-sm text-emerald-300">${l==='en'?'pts':'点'}</span></div><p class="text-xs text-gray-500 mb-5">${l==='en'?'Max combo':'最大コンボ'}: ${catchState.maxCombo}x</p><div class="flex gap-2"><button onclick="startHeartCatcher()" class="flex-1 btn btn-rose">${l==='en'?'Play Again':'もう一度'}</button><button onclick="closeGameModal()" class="flex-1 btn btn-ghost">${l==='en'?'Close':'閉じる'}</button></div></div></div>`);if(s>=30)setTimeout(()=>{const c=document.getElementById('catch-confetti');if(c)spawnConfetti(c)},200)},300)}

// ═══════════════════════════════════════
// BLOG / ARTICLES
// ═══════════════════════════════════════
const blogPosts={en:{1:{title:"5 Tips for a Great First Message",cat:"Dating Tips",color:"rose",content:`<p class="mb-4 text-sm text-gray-600 dark:text-gray-300 leading-relaxed">Your first message sets the tone for everything. Here are five tips that work:</p><div class="space-y-4"><div class="flex gap-3"><div class="w-7 h-7 bg-rose-50 dark:bg-rose-900/15 rounded-lg flex items-center justify-center shrink-0 mt-0.5"><span class="text-rose-500 text-xs font-bold">1</span></div><div><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">Be Specific</h4><p class="text-sm text-gray-500 dark:text-gray-300">Reference something from their profile. It shows you actually looked.</p></div></div><div class="flex gap-3"><div class="w-7 h-7 bg-rose-50 dark:bg-rose-900/15 rounded-lg flex items-center justify-center shrink-0 mt-0.5"><span class="text-rose-500 text-xs font-bold">2</span></div><div><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">Ask a Question</h4><p class="text-sm text-gray-500 dark:text-gray-300">Open-ended questions get the best responses.</p></div></div><div class="flex gap-3"><div class="w-7 h-7 bg-rose-50 dark:bg-rose-900/15 rounded-lg flex items-center justify-center shrink-0 mt-0.5"><span class="text-rose-500 text-xs font-bold">3</span></div><div><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">Keep It Light</h4><p class="text-sm text-gray-500 dark:text-gray-300">Be fun and easygoing. Save deep topics for later.</p></div></div><div class="flex gap-3"><div class="w-7 h-7 bg-rose-50 dark:bg-rose-900/15 rounded-lg flex items-center justify-center shrink-0 mt-0.5"><span class="text-rose-500 text-xs font-bold">4</span></div><div><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">Show Personality</h4><p class="text-sm text-gray-500 dark:text-gray-300">Let your unique character shine through.</p></div></div><div class="flex gap-3"><div class="w-7 h-7 bg-rose-50 dark:bg-rose-900/15 rounded-lg flex items-center justify-center shrink-0 mt-0.5"><span class="text-rose-500 text-xs font-bold">5</span></div><div><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">Be Respectful</h4><p class="text-sm text-gray-500 dark:text-gray-300">A kind, thoughtful message always stands out.</p></div></div></div>`},2:{title:"Understanding Japanese Dating Culture",cat:"Culture",color:"violet",content:`<p class="mb-4 text-sm text-gray-600 dark:text-gray-300 leading-relaxed">Dating in Japan has unique traditions. Understanding them helps you connect better.</p><div class="space-y-3"><div class="p-3 bg-violet-50 dark:bg-violet-900/10 rounded-lg"><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">&#127800; Kokuhaku (Confession)</h4><p class="text-sm text-gray-500 dark:text-gray-300">Relationships begin with a formal confession of feelings.</p></div><div class="p-3 bg-violet-50 dark:bg-violet-900/10 rounded-lg"><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">&#127803; Seasonal Dating</h4><p class="text-sm text-gray-500 dark:text-gray-300">Couples enjoy hanami, fireworks, fall foliage, and Christmas lights.</p></div><div class="p-3 bg-violet-50 dark:bg-violet-900/10 rounded-lg"><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">&#127873; Gift-Giving</h4><p class="text-sm text-gray-500 dark:text-gray-300">Valentine's Day and White Day are major occasions for couples.</p></div><div class="p-3 bg-violet-50 dark:bg-violet-900/10 rounded-lg"><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">&#128156; Taking It Slow</h4><p class="text-sm text-gray-500 dark:text-gray-300">Building trust and understanding is highly valued.</p></div></div>`},3:{title:"Building Confidence for Better Connections",cat:"Self Growth",color:"amber",content:`<p class="mb-4 text-sm text-gray-600 dark:text-gray-300 leading-relaxed">Confidence is one of the most attractive qualities. Build it naturally:</p><div class="space-y-4"><div class="flex gap-3"><div class="w-7 h-7 bg-amber-50 dark:bg-amber-900/15 rounded-lg flex items-center justify-center shrink-0 mt-0.5"><span class="text-amber-500 text-sm">&#128170;</span></div><div><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">Practice Self-Care</h4><p class="text-sm text-gray-500 dark:text-gray-300">Taking care of yourself creates a strong foundation.</p></div></div><div class="flex gap-3"><div class="w-7 h-7 bg-amber-50 dark:bg-amber-900/15 rounded-lg flex items-center justify-center shrink-0 mt-0.5"><span class="text-amber-500 text-sm">&#127919;</span></div><div><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">Set Small Goals</h4><p class="text-sm text-gray-500 dark:text-gray-300">Start by sending one new message each day.</p></div></div><div class="flex gap-3"><div class="w-7 h-7 bg-amber-50 dark:bg-amber-900/15 rounded-lg flex items-center justify-center shrink-0 mt-0.5"><span class="text-amber-500 text-sm">&#10024;</span></div><div><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">Embrace Uniqueness</h4><p class="text-sm text-gray-500 dark:text-gray-300">Your unique qualities are your biggest strengths.</p></div></div><div class="flex gap-3"><div class="w-7 h-7 bg-amber-50 dark:bg-amber-900/15 rounded-lg flex items-center justify-center shrink-0 mt-0.5"><span class="text-amber-500 text-sm">&#128214;</span></div><div><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">Learn from Experience</h4><p class="text-sm text-gray-500 dark:text-gray-300">Every interaction teaches you something new.</p></div></div></div>`}},jp:{1:{title:"最初のメッセージを成功させる5つのコツ",cat:"デートのコツ",color:"rose",content:`<p class="mb-4 text-sm text-gray-600 dark:text-gray-300 leading-relaxed">最初のメッセージがすべての始まりです。効果的な5つのコツ：</p><div class="space-y-4"><div class="flex gap-3"><div class="w-7 h-7 bg-rose-50 dark:bg-rose-900/15 rounded-lg flex items-center justify-center shrink-0 mt-0.5"><span class="text-rose-500 text-xs font-bold">1</span></div><div><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">具体的に</h4><p class="text-sm text-gray-500 dark:text-gray-300">プロフィールから話題を見つけましょう。</p></div></div><div class="flex gap-3"><div class="w-7 h-7 bg-rose-50 dark:bg-rose-900/15 rounded-lg flex items-center justify-center shrink-0 mt-0.5"><span class="text-rose-500 text-xs font-bold">2</span></div><div><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">質問をする</h4><p class="text-sm text-gray-500 dark:text-gray-300">オープンな質問が返信率を上げます。</p></div></div><div class="flex gap-3"><div class="w-7 h-7 bg-rose-50 dark:bg-rose-900/15 rounded-lg flex items-center justify-center shrink-0 mt-0.5"><span class="text-rose-500 text-xs font-bold">3</span></div><div><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">軽い雰囲気で</h4><p class="text-sm text-gray-500 dark:text-gray-300">最初は楽しく気軽に。</p></div></div><div class="flex gap-3"><div class="w-7 h-7 bg-rose-50 dark:bg-rose-900/15 rounded-lg flex items-center justify-center shrink-0 mt-0.5"><span class="text-rose-500 text-xs font-bold">4</span></div><div><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">自分らしさを出す</h4><p class="text-sm text-gray-500 dark:text-gray-300">個性を言葉で表現しましょう。</p></div></div><div class="flex gap-3"><div class="w-7 h-7 bg-rose-50 dark:bg-rose-900/15 rounded-lg flex items-center justify-center shrink-0 mt-0.5"><span class="text-rose-500 text-xs font-bold">5</span></div><div><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">礼儀正しく</h4><p class="text-sm text-gray-500 dark:text-gray-300">丁寧なメッセージは目立ちます。</p></div></div></div>`},2:{title:"日本のデート文化を理解する",cat:"文化",color:"violet",content:`<p class="mb-4 text-sm text-gray-600 dark:text-gray-300 leading-relaxed">日本のデート文化の伝統を理解しましょう：</p><div class="space-y-3"><div class="p-3 bg-violet-50 dark:bg-violet-900/10 rounded-lg"><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">&#127800; 告白文化</h4><p class="text-sm text-gray-500 dark:text-gray-300">正式な気持ちの告白から交際が始まります。</p></div><div class="p-3 bg-violet-50 dark:bg-violet-900/10 rounded-lg"><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">&#127803; 季節のデート</h4><p class="text-sm text-gray-500 dark:text-gray-300">花見、花火、紅葉、イルミネーションを楽しみます。</p></div><div class="p-3 bg-violet-50 dark:bg-violet-900/10 rounded-lg"><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">&#127873; 贈り物</h4><p class="text-sm text-gray-500 dark:text-gray-300">バレンタインとホワイトデーは重要なイベントです。</p></div><div class="p-3 bg-violet-50 dark:bg-violet-900/10 rounded-lg"><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">&#128156; ゆっくりと</h4><p class="text-sm text-gray-500 dark:text-gray-300">信頼を築くことが大切です。</p></div></div>`},3:{title:"より良い出会いのために自信をつける",cat:"自己成長",color:"amber",content:`<p class="mb-4 text-sm text-gray-600 dark:text-gray-300 leading-relaxed">自信は最も魅力的な資質です：</p><div class="space-y-4"><div class="flex gap-3"><div class="w-7 h-7 bg-amber-50 dark:bg-amber-900/15 rounded-lg flex items-center justify-center shrink-0 mt-0.5"><span class="text-amber-500 text-sm">&#128170;</span></div><div><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">セルフケア</h4><p class="text-sm text-gray-500 dark:text-gray-300">心身のケアは自信の土台です。</p></div></div><div class="flex gap-3"><div class="w-7 h-7 bg-amber-50 dark:bg-amber-900/15 rounded-lg flex items-center justify-center shrink-0 mt-0.5"><span class="text-amber-500 text-sm">&#127919;</span></div><div><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">小さな目標</h4><p class="text-sm text-gray-500 dark:text-gray-300">毎日1通のメッセージから。</p></div></div><div class="flex gap-3"><div class="w-7 h-7 bg-amber-50 dark:bg-amber-900/15 rounded-lg flex items-center justify-center shrink-0 mt-0.5"><span class="text-amber-500 text-sm">&#10024;</span></div><div><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">個性を受け入れる</h4><p class="text-sm text-gray-500 dark:text-gray-300">ユニークな性質が最大の強みです。</p></div></div><div class="flex gap-3"><div class="w-7 h-7 bg-amber-50 dark:bg-amber-900/15 rounded-lg flex items-center justify-center shrink-0 mt-0.5"><span class="text-amber-500 text-sm">&#128214;</span></div><div><h4 class="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-0.5">経験から学ぶ</h4><p class="text-sm text-gray-500 dark:text-gray-300">すべてのやり取りから学べます。</p></div></div></div>`}}};
