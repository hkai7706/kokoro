...
<div class="max-w-5xl mx-auto">
    <div class="mb-5">
        ...
        <p class="text-sm text-gray-500 mt-0.5" data-en="When they like you back, it's a match" data-jp="相手もいいねしてくれたらマッチ成立">When they like you back, it's a match</p>
    </div>
            ...
            @foreach($likedUsers as $likedUser)
                <div class="card overflow-hidden animate-in" style="animation-delay:{{ $loop->index * 0.04 }}s">
                    <a href="{{ route('user.profile', $likedUser->id) }}" class="block">
                        <div class="relative h-48 bg-gray-100 dark:bg-gray-800">
                            @if($likedUser->profile && $likedUser->profile->profile_photo)
                                <img src="{{ asset('storage/' . $likedUser->profile->profile_photo) }}" class="w-full h-full object-cover" loading="lazy" alt="{{ $likedUser->name }}'s profile photo">
                            @else
                    ...
                    <div class="p-3.5">
                        ...
                        @if($likedUser->profile)<p class="text-[11px] text-gray-500 mt-0.5">{{ $likedUser->profile->location }}</p>@endif
    ...
    @else
        <div class="text-center py-20">
            ...
            <p class="text-xs text-gray-500 mb-4" data-en="Browse profiles and like people you're interested in." data-jp="プロフィールを見て気になる人にいいねしましょう。">Browse profiles and like people you're interested in.</p>
